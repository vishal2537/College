...
int main(int argc, char const *argv[])
{
    ...
    printf("\nenter number of sequence to SKIP : ");
    int j = 0;
    int inp;
    scanf("%d", &inp);
    SKIPMAX = inp;
    ...
    while (1)
    {
        ...
        {
            if (frame.frame_no != noisy_ack_flag)
            {
                printf("\nS.NO. %d -- S. NO. : %d RECIEVED.\n", frame.frame_no, (frame.frame_no) % 2);
            }
        }
        if (frame.frame_no != noisy_ack_flag)
        {
        ...
        if (frame.frame_no == noisy_ack_flag && j <= SKIPMAX)
        {
            printf("\n*************** FRAME LOST REQUEST AGAIN ***********\n");
            if (j == SKIPMAX)
                noisy_ack_flag = -1;
            else
                noisy_ack_flag = arr[j++];
